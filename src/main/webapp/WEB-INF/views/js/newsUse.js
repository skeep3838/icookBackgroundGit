...
let json;
let topColumn = "<table class='table table-hover'><tr><th>公告ID"
	+ "	<th>公告標題<th>公告內容<th>創建時間<th>修改時間<th><th>";
let newsPageJson;
...
//					+ "value='取消新增' onclick='insertHide()'>";
			...
			for(let i=0 ; i<newsPageJson.length ; ++i){
				//顯示用資訊
				tableContant	+= 	"<tr id='showTr"+ i +"'><td>" + newsPageJson[i].maId
								+	"<td>" + newsPageJson[i].accrount
								+	"<td>***" 
								+	"<td>" + newsPageJson[i].accrountAuth
								+	"<td>" + newsPageJson[i].productAuth
								+	"<td>" + newsPageJson[i].orderAuth
								+	"<td>" + newsPageJson[i].webMaintainAuth
				...
				tableContant	+= 	"<tr id='updateTr"+ i +"' class='hideTr'><td><span>更新資料:</span>"
								...
								+	"<td><input size='12' type='text' id='accrount" + i + "' name='accrount' value='"
									+	newsPageJson[i].accrount + "'><br><span id='accountMeg'></span>"
				...
				for(let j=0 ; j < authName.length ; ++j){
					...
					for(let k=0 ; k < authArray.length ; ++k){
						tableContant 	+=	"<option value='"+ authArray[k] +"'"
										+ 	((newsPageJson[i][authName[j]] === authArray[k])?(" selected"):("")) 
